...
"""Usage reporting: tell the trace service that Overwinter was started.

Overwinter reports two events to https://trace.danielstephenson.dev through
the vendored trace client (overwinter/trace_client.py): ``startup`` once per launch and
...
``OVERWINTER_USAGE_REPORTING_ENABLED=false`` in the environment (see
...
the client itself, in its constructor, before Overwinter's own setting - so they
...
# The name the trace service issued Overwinter's key for. It is the `application`
# value on every event and must not change without a new key.
PROGRAM_NAME = "Overwinter"
...
OPT_OUT_INSTRUCTION = (
    "OVERWINTER_USAGE_REPORTING_ENABLED=false or TRACE_USAGE_REPORTING=off "
    "in the environment"
)
...
def createClient(config):
    ...
    TRACE_USAGE_REPORTING / DO_NOT_TRACK in the environment, then Overwinter's own
...
def showNoticeOnce(config, output=None):
    ...
    Overwinter already keeps state between runs. If it cannot be written the
